...
export default function Envelope({ onOpen }: EnvelopeProps) {
  ...
  return (
    ...
    >
      {/* Envelope Body */}
      <div className="relative w-80 h-56 bg-gradient-to-br from-pink-100 to-pink-200 rounded-lg shadow-2xl overflow-hidden">
        {/* Envelope Flap */}
        <motion.div
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <Heart className="w-12 h-12 text-pink-400 opacity-50" fill="currentColor" />
          </div>
        </motion.div>
        ...
        <div className="absolute bottom-4 left-0 right-0 flex flex-col items-center">
          ...
          <p className="text-pink-600 font-medium text-sm">Abre la Invitacion Mi Amor</p>
      ...
      >
        <Heart className="w-5 h-5 text-pink-300 opacity-50" fill="currentColor" />
      </motion.div>
    </motion.div>
  )
}